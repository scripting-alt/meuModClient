...
if $cygwin ; then
    ...
    [ -z "$ROOTDIRSRAW" ] && ROOTDIRSRAW=`find -L / -maxdepth 3 -type d -name gradle 2>/dev/null | head -n 1`
    ROOTDIR=`echo "$ROOTDIRSRAW" | head -n 1`
    if [ ! -z "$ROOTDIR" ] ; then
        ROOTDIR=`echo "$ROOTDIR"| sed 's|/gradle||'`
        [ ! -z "$ROOTDIR" ] && CLASSPATH="$ROOTDIR/.gradle/wrapper/dists/gradle-*/wrapper/gradle-*/lib/gradle-*.jar"
    fi
fi

# Capture the output and exit status of the first java command
